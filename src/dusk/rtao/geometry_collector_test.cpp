...
static AuroraGxCaptureDraw make_triangle_draw(std::vector<uint8_t>& verts,
    ...
    d.hasPnMtxIdx = false;
    d.currentPnMtx = 0;
    // identity rotation + non-zero translation: the collector rejects draws whose
...
TEST_CASE("GeometryCollector: raw_triangles returns captured geometry") {
    ...
    CHECK(gc.raw_triangles().size() == 2);
    gc.end_frame();
    // still accessible immediately after end_frame (cleared on next process_draw)
    CHECK(gc.raw_triangles().size() == 2);
}
